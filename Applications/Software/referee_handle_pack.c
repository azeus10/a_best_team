...
void Required_Data() //后续可根据需要自行添加
{
		...
		REFEREE_DATA.Barrel_Heat = power_heat_data.shooter_42mm_barrel_heat;		                //42mm热量读取
		REFEREE_DATA.CooLing_Value = robot_status.shooter_barrel_cooling_value;		
		REFEREE_DATA.Heat_Limit =  robot_status.shooter_barrel_heat_limit;	                    //发射机构热量上限
		REFEREE_DATA.Chassis_Voltage =  power_heat_data.chassis_voltage;                        //底盘电压
		REFEREE_DATA.Chassis_Current =  power_heat_data.chassis_current;                        //底盘电流
