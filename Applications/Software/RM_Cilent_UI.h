...
extern uint8_t press_refrsh_flag; // 额外声明标志位，在手动刷新完成之后在开始自动刷新任务
...
void ShootDeltaRefresh(float Up,float Left,float Right);  //摩擦轮转速变化量
void DistanceRefresh(float dis);                         //测距仪更新
void PitchYawRefresh(fp32 pitch,fp32 yaw);               //pitch，yaw轴角度更新

void shoot_refresh(int16_t status);       							//射击更新
void HeatLimitRefresh(uint8_t status) ;   							//热量限制显示
