...
void DM_start(uint8_t mode,can_id Motor_ID);//内部调用

/**********************************用户调用函数**************************************/
/*
	达秒电机的初始化函数
*/
void DM_Init()//使用MD电机需要在这初始化才能正常使用
{
	DM_start(2,CAN_2_2);
...
mode:模式：1.MIT模式,2速度位置模式，3速度模式,4.E-MIT模式
Motor_ID：达秒电机返回来数据的ID
*/
...
{
	if(DM_Mode[Motor_ID] == 2)//位置速度模式
	{
		DM_CAN_location_TX(Motor_ID,pos,vel);
	}
	else if(DM_Mode[Motor_ID] == 3)//速度模式
...
//		motor_data[master_ID].angle = rad2degree(uint_to_float(DM_motor_data[master_ID].p_int, P_MIN, P_MAX, 16));// 没仔细看达妙电机数据怎么解算的，先这样用
